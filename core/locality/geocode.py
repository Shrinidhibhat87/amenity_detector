...
Turns a free-text location (PIN code, street, or Stadtteil) into a coordinate
plus a bounding box, using the free OpenStreetMap Nominatim service.
...
class GeocodeClient:
    ...
    def geocode(self, query: str) -> GeocodeResult | None:
        """Resolve ``query`` to a coordinate, or ``None`` if Nominatim finds nothing.

        Raises :class:`GeocodeError` only when the service is unreachable after
        all retries — an empty result set is a normal ``None``, not an error.
        """
