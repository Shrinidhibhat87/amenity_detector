...
class PropertyAmenitySystem:
    ...
    def __init__(self, config: DictConfig, logger=None):
        ...
        # Resolve optional model weight directory from config; cast to str since Hydra returns Any.
        model_weight_dir: str | None = (
            str(config.output.model_weight_dir)
            if hasattr(config.output, "model_weight_dir")
            else None
        )
        self.detector = AmenityDetector(
            ...
            logger=self.logger,
            save_dir=model_weight_dir,
        )

        self.data_manager = AmenityDataManager(
            config.output.directory, amenity_schema=self.amenity_schema, logger=logger
        )
    ...
    def process_image(
        self, image_path: str
    ) -> tuple[dict[str, dict[str, bool]], str, dict[str, bool]]:
        """
        ...
        Returns:
            ...
            - amenities_by_room: room-keyed dict of amenity → presence
            - description: VLM-generated property description
            - detected_amenities: flat dict of amenity name → bool
        """
        self.logger.info(f"Processing image: {image_path}")

        # Detect amenities
        amenities_by_room, description, detected_amenities = self.detector.detect_amenities(
            image_path
        )

        # Save results
        self.data_manager.save_results(
            image_path, amenities_by_room, description, detected_amenities
        )
    ...
    def process_directory(self, directory_path: str) -> pd.DataFrame:
        ...
        for img_path in image_paths:
            try:
                amenities_by_room, description, detected_amenities = self.process_image(
                    str(img_path)
                )
    ...
    def process_image_from_memory(
        self, image: Image.Image, image_name: str
    ) -> tuple[dict[str, dict[str, bool]], str, dict[str, bool]]:
        """
