...
class AmenityDataManager:
    ...
    def _save_to_csv(
        ...
        # Values are str for metadata fields and int (0/1) for amenity presence.
        flat_data: dict[str, str | int] = {
        ...
        if file_exists:
            ...
            if new_columns:
                # Add new columns to existing fieldnames (convert Sequence to list first)
                updated_fieldnames = list(existing_fieldnames) + new_columns
            ...
            all_amenities = sorted(
                {a for amenities in self.amenity_schema.values() for a in amenities}
            )
    ...
    def get_results_summary(self) -> pd.DataFrame:
        ...
        try:
            ...
            return pd.DataFrame(
                columns=["image_name", "image_path", "description", "amenity_count"]
            )
        ...
        try:
            ...
            try:
                ...
                images_df = pd.read_sql_query(
                    "SELECT id, image_path, description FROM images", conn
                )

                # Get all amenities
                amenities_df = pd.read_sql_query(
                    "SELECT image_id, room_type, amenity_name, is_present FROM amenities", conn
                )
                ...
                if not amenities_df.empty:
                    ...
                    # Flatten MultiIndex column names (room_type, amenity_name) → "room_amenity".
                    # pivot_table with multiple grouping keys gives a MultiIndex of tuples.
                    # We cast each column to a tuple explicitly so mypy understands the unpack.
                    pivot_df.columns = [  # type: ignore[assignment]
                        f"{room}_{amenity}"
                        for room, amenity in (tuple(col) for col in pivot_df.columns)
                    ]
