...
class LlavaModel:
    ...
    def __init__(self, model_name: str = "llava-hf/llava-1.5-7b-hf", logger=None, save_folder=None):
        ...
        save_folder = (
            ...
            else f"/home/s.bhat/Coding/amenity_detection/model/LlavaModel/{model_name}"
        )
    ...
    def generate_description(self, img: Image.Image, detected_amenities: dict[str, bool]) -> str:
        """
        ...
        present_amenities = [
            amenity for amenity, is_present in detected_amenities.items() if is_present
        ]
        ...
        prompt = (
            "Generate a natural and appealing property description highlighting these amenities:\n"
        )
        prompt += ", ".join(present_amenities)
        prompt += "\n\nBased on these amenities, classify if this is propery image is a kitchen or a living room or "
        "a bathroom, or a bedroom, and include that in your description."
        ...
        conversation = [
            ...
            },
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": "Please describe the property and highlight the amenities.",
                    },
                ],
            },
        ]
        ...
        # These are guaranteed to be set by _load_model; assert so mypy knows they aren't None.
        assert self.processor is not None, "Processor was not loaded"
        assert self.model is not None, "Model was not loaded"
    ...
    def detect_amenities(self, image: Image.Image, all_amenities: list[str]) -> dict[str, bool]:
        ...
        conversation = [
            ...
            },
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": "Please follow the prompt and generate JSON like output.",
                    },
                ],
            },
        ]
        ...
        try:
            ...
            if json_start == -1 or json_end == 0:
                if self.logger:
                    self.logger.error("Could not find JSON like object in response.")
                # Return empty dict immediately — no JSON to parse.
                return {}
            ...
            detected_amenities: dict[str, bool] = json.loads(json_text)
