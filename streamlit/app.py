...
class SteramlitConfigWrapper:
    ...
    def process_image_directly(
        self, image
    ) -> tuple[dict[str, dict[str, bool]], str, dict[str, bool], float]:
        """
        ...
        # Process the image
        amenities_by_room, description, detected_amenities = system.process_image_from_memory(
            image, image_name
        )
            ...
            if st.button("Detect Amenities"):
                with st.spinner("Processing image..."):
                    if api_mode == "Direct Inference":
                        ...
                        amenities_by_room, description, detected_amenities, processing_time = (
                            streamlitconfig.process_image_directly(image)
                        )
                    else:
                        # Use the API
                        amenities_by_room, description, processing_time = process_image_via_api(
                            uploaded_file
                        )
...
def process_image_via_api(uploaded_file) -> tuple[dict[str, dict[str, bool]], str, float]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(f"API Error: {response.status_code} - {response.text}")  # type: ignore[attr-defined]
            return {}, "Error processing image", processing_time

    except Exception as e:
        st.error(f"Error: {str(e)}")  # type: ignore[attr-defined]
