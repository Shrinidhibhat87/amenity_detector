...
async def detect_amenities(
    file: UploadFile = File(...), amenity_system=Depends(get_amenity_system)
):
    """
    ...
    """
    # Validate file — content_type can be None if the client doesn't set it.
    ...
    try:
        ...
        # `raise ... from e` tells Python (and linters) that this new exception was caused
        # by `e`, which makes tracebacks clearer when debugging.
...
def get_results(amenity_system=Depends(get_amenity_system)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting results: {str(e)}") from e
